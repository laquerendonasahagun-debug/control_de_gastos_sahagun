...
function ensureSchema(sql) {
  if (!schemaPromise) {
    schemaPromise = sql`
      CREATE TABLE IF NOT EXISTS sahagun_expenses (
        id TEXT PRIMARY KEY,
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === 'DELETE') {
      ...
      const deleted = await sql`DELETE FROM sahagun_expenses WHERE id = ${id} RETURNING id`;
